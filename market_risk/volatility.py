...
class GARCHEngine:
    ...
    def fit(self, returns: pd.Series) -> None:
        """
        Fits the GARCH(p,q) model to the return series.
        """
        # arch_model expects returns to have zero mean if mean='Zero'
        am = arch_model(returns, vol='Garch', p=self.p, q=self.q, dist=self.dist, mean='Zero', rescale=False)
